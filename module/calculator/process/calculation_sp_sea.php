<table class='table table-sm'>
  ...
  <tbody>
    <tr>
      ...
      <th class='text-right'>
        <?= number_format(CALCULATOR::Calc_Sea_SP_Hub_Fee($total_chr_value), 2); ?>

      </th>
    </tr>
    <tr>
      <td>
        <?= $language[$locale]["PRODUCT_SEA_SP_DELIVERY"]; ?> 
        <br><small>(<?= $language[$locale]["MSG_MIN_SPEND"] . CALCULATOR::$min_kg_sea_sp_delivery . " KG</small>)";?>
      </td>
      <th class='text-right'>
        <?php $total_price = CALCULATOR::Calc_Sea_SP_Delivery_Fee($total_chr_value) + CALCULATOR::Check_Over_Length_Fee($max_length); ?>
        <?= number_format($total_price, 2); ?>
        <br>
        <?php CALCULATOR::Gen_Over_Length_icon($max_length, $language[$locale]["MSG_INC_OVER_LENGTH_FEE"]); ?>
    ...
    <!-- <tr>
      ...
      <th class='text-right'>
        <?php if (CALCULATOR::Calc_Sea_SP_DDP($total_chr_value) == 0) {
