<?php
	...
	function ChartGenerate(
		...
		echo "<div style='height:" . $height . "px';>
			<canvas id='myChart_" . $shortname . "'></canvas>
		</div>
		...
		<script>" ;
			...
			//setup
			//label
			echo "const labels_" . $shortname . " = [" ;

				for ( $count = 0 ; $count < ($count_data - 1) ; $count++) {
					echo $array_data[$count][0] . "," ;
				}
				echo $array_data[($count_data - 1)][0] ;

			echo "];
			const data_" . $shortname . " = {
				labels: labels_" . $shortname . ",
				datasets:
				[
					{
						label: '" . $name_en . "',
						...
						[" ;
							...
							for ( $count = 0 ; $count < ($count_data - 1) ; $count++) {
								echo $array_data[$count][$col_no] . "," ;
							}
							echo $array_data[($count_data - 1)][$col_no] ;
					...
					if($array_data2<>""){
						...
						,
						{
							...
							[" ;
								...
								for ( $count = 0 ; $count < ($count_data - 1) ; $count++) {
									echo $array_data2[$count][$col_no2] . "," ;
								}
								echo $array_data2[($count_data - 1)][$col_no2] ;
			...
			//config
			echo "
				const config_" . $shortname . " = {
					type: '" . $chart_type . "',
					data: data_" . $shortname . ",
					options: {
						showTooltips: false,
						responsive: true,
			...
			//render the chart
			  echo "var myChart_" . $shortname . " = new Chart(
				document.getElementById('myChart_" . $shortname . "'),
				config_" . $shortname ;
			  echo ");
		</script>" ;
	}
